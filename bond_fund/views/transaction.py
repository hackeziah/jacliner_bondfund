...
# def data_viewby(request):
#     if request.method == "GET":
#         if request.is_ajaxI():
#             status =
#             data = Transaction.objects.filter(trash=0,status = ).order_by('-date_created')
...
def create_transaction(request,request_no):
    ...
    if request.method == "GET":
        try:
            data_request = Requests.objects.get(request_no = str(request_no), trash = 0,status = 1)
            account_no = data_request.account_no
            data_account = Account.objects.get(account_no = str(account_no),trash = 0)
...
def deduction_transaction_view(request):
    ...
    data = Transaction.objects.filter(trash=0,ttype=2).order_by('date_created')
...
def saving_transaction_view(request):
    ...
    data = Transaction.objects.filter(trash=0,ttype=1).order_by('date_created')
...
def view_transaction(request):
    if request.method == "GET":
        ...
        if request.is_ajax():
            data_transaction = Transaction.objects.get(id = int(transaction_id),trash=0)
            ...
            employee_name = data_transaction.account.empno.last_name + ", " +data_transaction.account.empno.first_name
