...
from account.models import Company, Position, UserAccount
...
def usermanagement_detail(request):
    if request.method == "GET":
        ...
        if request.is_ajax():
            ...
            account_id = data.id
            requests = Requests.objects.filter(account_no=str(account)).values(
                'amount', 'request_no', 'transactionttype', 'transactiontname', 'date_created').order_by('-date_created')
            transactions = Transaction.objects.filter(account_id=int(account_id)).values(
                'transaction_no', 'amount', 'last_balance', 'current_balance', 'transactionttype', 'date_created').order_by('-date_created')
...
def add_user(request):
    if request.method == 'POST':
        ...
        message = "This is the password for this Account " + \
            codeAccount + " password is " + genPassword
...
def user_add(request):
    title = "Add User"
    contentheader = "Creating User"
    user = request.session['user']
    data = Account.objects.filter(trash=0)
    position_list = Position.objects.filter(trash=0)
    company_list = Company.objects.filter(trash=0)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ua = UserInfo.objects.latest('id')
            # `id`, `account_no`, `balance`, `status`, `trash`, `empno_id`
    ...
    return render(request, 'usermanagement/add-user.html', content)
